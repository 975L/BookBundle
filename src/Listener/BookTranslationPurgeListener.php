...
// Takes a row's translations away with the row, as ShopBundle's ShopTranslationPurgeListener does: they name their owner rather than pointing at it, so no foreign key does it for them - the trash being no removal, a row set aside keeps its own
#[AsDoctrineListener(event: Events::preRemove)]
#[AsDoctrineListener(event: Events::postRemove)]
class BookTranslationPurgeListener
{
    ...
    // What a row's translations are filed under, null for anything this catalog does not translate
    private function owner(object $entity): ?string
    {
        return match (true) {
            $entity instanceof Book => BookTranslator::OWNER_BOOK,
            $entity instanceof Serie => BookTranslator::OWNER_SERIE,
            $entity instanceof BookCategory => BookTranslator::OWNER_CATEGORY,
            $entity instanceof Strip => BookTranslator::OWNER_STRIP,
