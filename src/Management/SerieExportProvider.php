...
class SerieExportProvider implements ExportProviderInterface
{
    ...
    private function exportSerieData(Serie $serie, array &$files): array
    {
        ...
        return [
            ...
            // Same keys as before the person became a row of their own, so an archive reads on either version (see ContributorResolver)
            'author' => $serie->getAuthor()?->getName(),
            'authorWebsite' => $serie->getAuthor()?->getWebsite(),
            'illustrator' => $serie->getIllustrator()?->getName(),
            'illustratorWebsite' => $serie->getIllustrator()?->getWebsite(),
