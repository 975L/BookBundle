...
// Resolves, for UiBundle's front-end "Edit this block" hover button, the EasyAdmin edit URL of the Book, Serie, Strip, Contributor or BookCategory owning a given Block - so the blocks composed at the foot of a display page hover the same pencil as the sections above them (see BookEditUrlExtension for those)
class BookBlockEditUrlProvider implements BlockEditUrlProviderInterface
{
    ...
    public function getEditUrls(array $blocks): array
    {
        ...
        $families = [
            ...
            ContributorCrudController::class => $this->contributorRepository->findByBlockIds($blockIds),
            BookCategoryCrudController::class => $this->categoryRepository->findByBlockIds($blockIds),
        ];
     ...
     */
    private function urlsOf(string $crudControllerFqcn, Book | BookCategory | Contributor | Serie | Strip $owner, array $blockIds): array
    {
        $urls = [];
