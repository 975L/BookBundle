...
// Publishing a new version of a book: the book keeps its address and its identity - its id, its slug, its readers' ratings, its blocks, its translations - and a twin takes the version being set aside, with its ISBN, its date, its files and its platforms. The twin only leaves the catalog: it keeps its page, its ISBN and its place in the sitemap, and is reached from the book replacing it (see Book::$newerVersion)
// The gesture sits here rather than in the screen or in a command, because it serves both: splitting an already published catalog, and publishing a new version of an existing book
class BookVersionPublisher
{
    ...
    // The twin carrying the version set aside: the same book, the same number, the same language, the same author. No translations, no blocks, no reader ratings - those belong to the page being kept, which is the one outside links reach
    // The title is the one the editor writes, in the book's language: the bundle presumes no reason for the replacement - original edition, unillustrated text, second edition - and that title is what names the cross link from one version to the other (see Book/Hero.html.twig)
    ...
    // Moves an edition onto another book: it is what carries the ISBN and the release date of the text as it came out. Its files and its platforms do not follow it - they belong to the book, and the pages are what leave with it (see moveMedias())
    // Written on the owning side only, never through removeEdition(): the editions collection is "orphanRemoval: true" (see Book::$editions), and taking a row out of it schedules it for deletion - reattaching it elsewhere in the same unit of work does not undo that. The target book must therefore already exist in database
