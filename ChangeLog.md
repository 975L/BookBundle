...
- **New `BookCategory` (table `book_category`, join table `book_category_link`)**: what a book is about, where its serie says what it belongs to - flat and many-to-many, a book carrying as many as it deserves and none being just as valid. Flat with a page of its own is what a tag is once it has an address, so the bundle ships one family and not two: a site naming them "Genres", "Thèmes" or "Rayons" overrides the `label.categor*` keys rather than storing a second graph (03/09/2026)
- **Nothing appears until a site asks for it**: the new `book-route-categories` entry ships empty, so a site upgrading to this version serves no category page at all - the names still read on a book's sheet, as plain words, which is what using them as tags amounts to. Filled in, the index and each category answer below that segment, like the series (03/09/2026)
- New back-office screen under "Catalogue", laid out like the series' one - drag to reorder, hide, trash, copy, exports, and the blocks composing the category's page. A book receives its categories from its own screen, in an autocomplete field (03/09/2026)
- New `book_categories` block kind, and the `book_books` kind now takes a category slug: a page can print "our novels" without a listing of its own (03/09/2026)
- A category, and a serie, holding only books still to come stops heading a page with an empty grid under it: the join reads the publication date too, which its page already did (`BookCategoryRepository::findWithBooks()`, `SerieRepository::findWithBooks()`) (04/09/2026)
- New optional `code` column, for the trade's own classifications - CLIL in France, Thema or BISAC abroad - asked of a publisher distributing through a wholesaler. Stored and never interpreted: a site publishing on its own leaves it empty (03/09/2026)
- The categories travel: their own export/import kind (`book_category`), the book's archive naming them by slug and creating the ones this environment doesn't hold yet (new `BookCategoryResolver`), the sitemap declaring the index and each category, the menu picker offering them as targets, and the structured data of a book naming them under `genre` (03/09/2026)
- New `label.categor*`, `label.block_categor*`, `label.gallery_showcase_book_categories*` and `flash.category_*`, in the three locales, `narration.categories` in the two the narration is written in, and `label.book_route_categories` with its description in the site config catalog (03/09/2026)
- **New `BookReleaseAlert` (table `book_release_alert`)**: an address left on the page of a book with no date, or with a date still to come, and told once when it appears. The link sits under the "à paraître" line of `Book:Hero` and leads to a page of its own - the sheet's html is handed to a shared cache per fragment, where a form needs a session and a csrf token - behind the honeypot and the hourly limiter (`book_release_alert`) every public form of the ecosystem is served under (03/09/2026)
- **One e-mail, and the row goes with it**: `c975l:book:release-alerts:send`, scheduled nightly by the bundle itself (new `BookMaintenanceTaskProvider`, so a site installing the catalog has nothing to add to its `MaintenanceSchedule`), writes to everyone whose book has come out and deletes the subscription as it does - a book is published once, so nothing is left to wait for and nothing left to keep. Which books count is `BookRepository::publishedQueryBuilder()`'s own reading, asked in the database: a book set aside, one of a serie set aside, one in the trash or one replaced by a newer version is not published (03/09/2026)
- The subscription is acknowledged at once by a first message, the only one sent before the parution and the only way out offered until then: it carries the unsubscribe link, so somebody whose address was typed by a third party leaves from there. The parution e-mail carries none, the row being gone by then (03/09/2026)
- The unsubscribe link opens a page carrying a button and takes nobody off the list on sight: the mail gateways walking every address of a message before it is read - Outlook's Safe Links, an antivirus - would otherwise unsubscribe the reader minutes after the acknowledgement reached them. New `label.release_alert_unsubscribe_*` and `text.release_alert_unsubscribe_confirm`, in the three locales (04/09/2026)
- The same run drops the lists of books announced and never published (24 months, `BookReleaseAlertService::PENDING_RETENTION_MONTHS`), and prints what is left waiting: a queue that stops going down is how a site finds out its mailer is refusing (03/09/2026)
- An address the mailer keeps refusing is counted (new `attempts` column) and let go after three nights, the queue being read by that count first: a handful of dead addresses filled every run's batch from the head of the queue and nobody behind them was ever served (04/09/2026)
- New `book_release_alert_confirmation` and `book_released` e-mail templates, declared in the three locales and seeded by `c975l:ui:email-templates:ensure`, sent under the site-wide `email-*` addresses - this bundle declares no address of its own (03/09/2026)
- New `Book::isToBePublished()`, the one reading behind the "à paraître" wording and behind what the alert is offered on, and new `Book::isShownInCatalog()`: an alert taken on a book the catalog has dropped - set aside, replaced, or held by a serie set aside - was acknowledged and then never sent, the nightly run reading the same conditions. New `text.release_alert_unavailable`, in the three locales (04/09/2026)
- New `label.release_alert_*`, `label.book_released_*` and `text.release_alert_*`, in the three locales (03/09/2026)
- **The other parts someone takes in a book are rows and no longer missing**: `BookContributor` (table `book_book_contributor`) joins a book, a person and the part they hold - narrating it, translating it - so the same person is a narrator on one title and a translator on the next without a column being added to every catalog. Author and illustrator stay columns of the book, being the two it takes from its serie when it names none (03/09/2026)
- The rows are written on the book's screen, under a "Participants" fieldset holding one line per person and per part, where the author and the illustrator are already picked - and `Book:Informations` prints them under those two, each name leading to their page. Written there and not on the person's screen, where they are only read: a voice reading twenty stories would have carried twenty lines on a screen that says who somebody is (03/09/2026)
- New `getContributorRoles()` on `BookCustomizationProviderInterface`: the parts a site credits, its own vocabulary as its files, its editions and its platforms already are. An empty array falls back to `BookContributorRole` - narrator and translator. New `book_role_label()`, which reads that vocabulary the way `book_edition_label()` reads the editions (03/09/2026) **Breaking: an app implementing the interface declares the method, returning `[]` to keep the bundle's own**
- A person's page lists what they only narrated or translated beside what they signed, and the index of people counts them as credited: someone recorded as a narrator alone had a page with nothing under it and was left out of the listing (`Contributor::getBooks()`, `ContributorRepository::findCredited()`) (03/09/2026)
- The trash asks them too: setting aside or deleting someone a book credits as its narrator is refused the way an author is (`Contributor::holdsContent()`) (03/09/2026)
- The credits travel with the book: the catalog export and import carry them, each row naming its person by name as the author is named, and a copied book is copied with them (03/09/2026)
- The structured data names the translator, which is the one part schema.org holds on a `CreativeWork` - the voice reading a book is `readBy`, which belongs to an `Audiobook` and not to the `Book` node the page publishes (03/09/2026)
- **A person says what they did**, under their name on their page and on their card in the listing: `Contributor::getRoles()` reads it from what credits them - the two columns of a book first, then the parts its rows name - rather than from a field of their own, so nobody types "author" on a row. New `.book-hero__roles`, `.book-card__roles`, `--book-roles-size`, `--book-roles-opacity` and `--book-roles-space` (03/09/2026)
- `BookCustomizationRegistry::getRoleLabel()` names a part wherever it is printed - a page, a screen - the author and the illustrator included, the two a book holds in columns of its own and no vocabulary declares. `book_role_label()` reads it (03/09/2026)
- A book set aside as a previous version keeps its credits: `BookVersionPublisher` copied the author and the illustrator onto the twin and left the rest behind (03/09/2026)
- The guided tour of a book's creation walks the new fieldset, between the author and the technical tab (03/09/2026)
- New `BookContributorType`, and new `label.book_contributor`, `label.book_contributors`, `label.book_contributors-help`, `label.role`, `label.role_narrator`, `label.role_translator`, `label.guided_step_book_creation_contributors` and `description.guided_step_book_creation_contributors`, in the three locales, plus `narration.guided_step_book_creation_contributors` in the two the narration is written in (03/09/2026)
- New `book-isbn-prefixes`: the ISBN roots the publisher was granted, one per line and hyphenated as they are printed. `|isbn` cut every number 3-2-5-2-1, which is only right under one prefix - where an ISBN is cut comes from the ranges the International ISBN Agency publishes and from nothing in the number itself, and a publisher holding "978-2-9598005" beside "978-2-488750" is cut at a different digit on each. A number matching no declared root now keeps its digits and no hyphen at all, hyphens laid wrong reading as another publisher's ISBN (03/09/2026)
- `Book:Informations` only writes an edition holding an ISBN, a format or a pagination: an edition opened for a recording not out yet printed its own name followed by a bare dash (03/09/2026)
- `Book:Extracts` hands the slider its heading translated in the book's language, and `Book:Resume` / `Serie:Resume` hand `Text:Readmore` the language the summary is written in: an English book read from a French site printed "Galerie de pages" over its own pages and "Lire la suite" under its own summary (03/09/2026)
- `Book:Hero`, `Book:Book`, `Serie:Hero`, `Strip:Hero` and `Strip:Card` hand `Rating:Rating` the language their work is written in: an English book read from a French site said "Aucun vote" under its own title, and the words its controller writes after a vote came in French too (03/09/2026)
- `Serie:Hero` joins what it credits instead of separating it with a fixed dash: a serie naming neither author nor illustrator opened its line on a stray dash before the number of books, where `Book:Hero` already guarded against it (03/09/2026)
- New `label.book_isbn_prefixes` and `description.book_isbn_prefixes`, in the three locales (03/09/2026)
- `Contributor:Hero` reads the sentences a person is introduced with, right under their name, and `Contributor:Resume` is gone with the block that printed them under the hero: the hero said "6 livres" over the very list printing those books, and a site naming its catalog "Toutes nos histoires" read "6 toutes nos histoires". The pencil over the hero opens their screen as it did, the summary being written on it. New `--book-hero-summary-space`, what separates two of its paragraphs (03/09/2026)
- A page opening straight on its hero holds it off the bar above: a person's page carries no trail, and the body of a site whose navbar is fixed only makes room for the bar's own height, so the portrait was printed against it. New `--book-hero-margin-block-start`, the gap the page already keeps between its blocks (03/09/2026)
- A person carries the platforms their books are bought at, written where a book's own are and printed the way a book prints them: a new "Buy" fieldset on their screen holds one row per platform (`ContributorLink`, table `book_contributor_link`), and `Contributor:Shops` lays them under `#shops` as tiles, the site's own shop at the head and accented. A page that said the same in hand-composed blocks - a button plus one card per store - is replaced by those rows (03/09/2026)
- The platforms travel with the person: the catalog export and import carry them, and a copied person is copied with them (03/09/2026)
- The people a catalog credits are listed centred rather than flushed left: three portraits against the left edge of a wide page read as a listing that failed to fill, where a shelf of books keeps its columns. New `.book-cards--centered` (03/09/2026)
- `Contributor:Contributor` prints the name once: the portrait was handed it as a visible label and the heading below wrote it a second time, in another font (03/09/2026)
- New `label.contributor_buy`, `label.contributor_links-help` and `label.contributor_link_url-help`, in the three locales (03/09/2026)
- `book-contributor-creation` walks that "Buy" fieldset, which it stopped at the portrait and saved past. New `label.guided_step_book_contributor_creation_links` and its description in the three locales, its narration in the two (04/09/2026)
- The pencil over a book's "Buy" and "Listen" cards opens the collection each is written in - `buyLinks` and `listenLinks` - where it still opened the editions: the platforms left the editions when each gesture took a collection of its own, and the two entries of the map stayed behind (03/09/2026)
- `Serie:Hero` says who signed the serie and no longer how many books it holds: the figure stood right over the list printing them, and it was said with `label.books`, the word a site also names its whole catalog with - one calling it "Toutes nos histoires" read "3 toutes nos histoires". A serie crediting nobody writes no line at all rather than an empty one (03/09/2026)
- The links health check walks the people as well as the books: someone's own page at a store rots exactly as a book's does, and nothing said so until a reader clicked. Each row leads to the collection the address is typed in - `buyLinks`, `listenLinks`, `watchLinks` on a book, `links` on a person - where every one of them opened the editions (03/09/2026)
- An address of the site itself, stored as the page writes it ("/shop/..."), is probed under the address the site declares in `site-url` instead of being reported as malformed - and reported as nothing to probe when the site declares none. New `label.health_check_link_internal`, in the three locales (03/09/2026)
- Every listing kind carries its own head - anchor, eyebrow, title, paragraph and colored flat, the very fields a `text_section` offers: a page had to place a text block above each listing, and the pair had to be moved, hidden and translated together. `book_series`, `book_books`, `book_contributors`, `book_to_be_published` and `book_serie_strips` share those fields through the new `AbstractBookListingBlockType` and draw them through the new `Listing:Section` component. New `.book-listing`, `.book-listing__intro` (03/09/2026)
- `book_to_be_published` prints no heading of its own any more: its words are typed on the block like every other listing's, so a page updating from an earlier version types them once. `Book:ToBePublished` itself is untouched - the pages calling the component directly keep their translated heading (03/09/2026)
- The listings draw at random: new `random` on the five kinds, shuffling the whole catalog before the maximum applies rather than shuffling what the maximum already kept. A block asking for it declines its own cache entry, so the draw is renewed at every render (03/09/2026)
- `book_to_be_published` takes a maximum, which it had no field for (03/09/2026)
- The five kinds declare `eyebrow, title, content` translatable, so a site reading in more than one language translates the heads its editor types (03/09/2026)
- `book-composition` says a listing carries its own head, which it was written before: an editor following the tour kept laying a `text_section` above the grid. Two steps, and new `label.guided_step_book_composition_listing`, `label.guided_step_book_composition_heading` with their descriptions in the three locales, their narrations in the two (04/09/2026)
- Its "add" step points at the button opening the palette rather than at the rows already there, the same correction `book-reader` took (04/09/2026)
- The suite covers every class the lot ships: the nightly command, the six listing kinds and their shared head, the three new form types, the categories' service and page, the block owners, and the edition, structured-data and catalog-block Twig functions (04/09/2026)
- The block forms are read in the `book` domain: their labels were written in French inside the FormTypes, where every other bundle's are translated (03/09/2026)
- New `label.anchor`, `label.anchor_help`, `label.block_eyebrow`, `label.block_title`, `label.block_content`, `label.block_max_books`, `label.block_max_series`, `label.block_max_contributors`, `label.block_max_strips`, `label.block_random`, `label.block_random_help`, `label.block_display_more_books`, `label.block_display_more_series`, `label.block_display_more_contributors`, `label.block_serie_slug` and the six `label.section_background*`, in the three locales (03/09/2026)
- `Book:ToBePublished`, `Book:BooksSection` and `Serie:SeriesSection` are gone, and with them `label.books_to_be_published`, `text.books_to_be_published`, `text.books_to_be_published_social`, `label.books_latest` and `label.series_ours`: each wrote a heading of its own over a grid the block kinds now draw under the one an editor types, and the one page still calling them composes that section in the back office (03/09/2026)
- The book sheet now states the site's age warning above its sections, for a book carrying an age: what `Book:Informations` only prints as a figure is said in words too, and in the book's own language on a site that translated the setting (see CoreBundle's `site-age-warning` config and `Alert:AgeWarning`) (03/09/2026) **Needs `c975l/core-bundle` ^1.22.1**
- **The four catalog indexes are describable from the back office**: `Management\UrlMetadataProvider` lists `/books`, `/series`, `/contributors` and `/strips` in *Descriptions d'urls* under the very prefixes the site serves them at, and each index now reads the title and the sentence written there over its own translated label (03/09/2026) **Needs `c975l/core-bundle` ^1.21.7**
- `Book:ToBePublished`, `Book:BooksSection`, `Book:Explanation`, `Serie:SeriesSection` and the series' index read their heading and their paragraph from the `book` domain, where the French was written into the template - a section rendered on every site posting the block, in whatever language it reads (03/09/2026)
- `Book:ToBePublished` takes an `anchor`, and only writes the link to the social networks when the page hands it one: it pointed at `#reseaux` on every site, an id no block carries - `social_links_display` holds no data of its own - so it led nowhere on a page composed in the back office (03/09/2026)
- New `label.books_to_be_published`, `label.books_latest`, `label.series_ours`, `label.series_all`, `label.series_summary`, `text.books_explanation`, `text.books_to_be_published` and `text.books_to_be_published_social`, in the three locales (03/09/2026)
- The `book-reader` step naming the cues points at `[data-ea-collection-field][data-prototype*="_data_cues_"]`: EasyAdmin's collection widget renders no id at all, so `[id$="_data_cues"]` highlighted nothing on screen and only the prototype's placeholder tells this collection from the block's medias (03/09/2026)
