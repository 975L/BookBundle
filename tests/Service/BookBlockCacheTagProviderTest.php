...
class BookBlockCacheTagProviderTest extends TestCase
{
    ...
    // No variant of a serie reads a date any more: the tile and the vignette printed how many books it had out until that line was dropped, and the three of them are now cached whatever is scheduled
    public function testEverySerieVariantIsCachedWhateverIsScheduled(): void
    {
        foreach (['', 'tiles', 'thumbnails'] as $variant) {
            $block = new Block()->setData(['variant' => $variant]);

            $this->assertSame([BookBlockCacheInvalidator::CACHE_TAG_CATALOG], $this->resolvers(true)['book_series']($block), $variant);
        }
    }
