...
namespace c975L\BookBundle\Tests\Translation;

use PHPUnit\Framework\TestCase;

// A narration is never drawn, so a key missing from its catalogue shows nowhere: the films read the fallback and nobody notices until one is filmed. Nothing else checks these - TranslationCatalogTest only sees keys passed to trans(), where a narration is declared in an array
class NarrationCatalogueTest extends TestCase
{
    // The bundle's own narrations, resolved in its "book" domain suffixed "_narration" - spoken and never drawn, hence the two locales the rest of the bundle does not stop at
    ...
    public function testEveryDeclaredNarrationIsShippedInBothLocales(): void
    {
        ...
        foreach (self::LOCALES as $locale) {
            ...
            foreach ($declared as $file => $keys) {
                foreach ($keys as $key) {
                    $this->assertContains($key, $catalogue, sprintf('"%s" declares "%s", which "book_narration.%s.xlf" does not ship.', $file, $key, $locale));
                }
            }
        }
    }
    ...
    public function testNoNarrationIsShippedWithoutBeingDeclared(): void
    {
        ...
        foreach ($this->catalogue('en') as $key) {
            $this->assertContains($key, $declared, sprintf('"book_narration.en.xlf" ships "%s", which nothing in "src/" declares any more.', $key));
        }
    }
    ...
    {
        $path = $this->root() . '/translations/book_narration.' . $locale . '.xlf';
