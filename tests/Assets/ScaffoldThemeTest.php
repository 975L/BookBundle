...
// The scaffolded book.css is a hand-maintained copy of the token defaults, so it drifts on its own
// Only this bundle's own tokens are asserted: the colors it reads from UiBundle are the site's palette, admin-editable and deliberately absent from both, and the summary of anchors standing beside a book page is UiBundle's --toc-*, offered by that bundle's own themes/ui.css
class ScaffoldThemeTest extends TestCase
{
    ...
    public function testScaffoldOffersEveryDeclaredToken(): void
    {
        ...
        $this->assertSame([], array_values($missing), sprintf(
            'sass/_variables.scss declares %s that the scaffolded book.css never mentions: a design has no way to discover them short of reading the compiled stylesheet.',
            implode(', ', $missing)
        ));
    }
    ...
    public function testScaffoldValuesMatchTheCompiledDefaults(): void
    {
        ...
        $this->assertSame([], $drifted, sprintf(
            "The scaffolded book.css no longer mirrors sass/_variables.scss:\n- %s",
            implode("\n- ", $drifted)
        ));
    }
    ...
    public function testScaffoldShipsEverythingCommentedOut(): void
    {
        ...
        $this->assertSame([], $matches[1], sprintf(
            'The scaffolded book.css declares %s outside a comment: a fresh site would freeze that value instead of following the bundle.',
            implode(', ', $matches[1])
        ));
    }

    // D. A "--book-" token read by a rule but declared nowhere resolves to nothing at all, silently - the compiled sheet is valid CSS either way, so only a typo hunt catches it
    ...
    /**
     * Every "--book-" token this bundle's own sass reads, whatever the rule reading it.
     *
    ...
    private function tokensReadFromSass(): array
    {
        ...
        foreach (glob($root . '/*.scss') ?: [] as $path) {
            ...
            preg_match_all('/var\(\s*(--book-[a-z0-9-]+)/', $code, $matches);
    ...
    {
        return \dirname(__DIR__, 2) . '/scaffold/assets/styles/themes/book.css';
    }
    ...
    private function scaffoldTokens(): array
    {
        ...
        $this->assertFileExists($path, 'The scaffolded book.css is missing, so nothing offers this bundle\'s tokens to a site.');
