...
The bundle also ships eight block kinds of its own. Six put a selection of the catalog on any page of the site — `book_series`, `book_categories`, `book_books`, `book_to_be_published`, `book_serie_strips` and `book_contributors`, which prints the people the catalog credits, each leading to their own page. `book_books` also takes a category slug, so a page prints "our novels" without a listing of its own, and says whether a phone scrolls its covers sideways as one rail rather than stacking the grid — past the phone both are that same grid. `book_serie_strips` names one serie or several, separated by commas, the maximum then applying to each of them, so one block shows as many planches of one serie as of the other. Set to UiBundle's `compact` variant, they print a book at a thumbnail's width, with its cover, its title and its language and without its summary.

`book_series` also takes the series to show, named by slug and separated by commas — left empty, the whole catalog, which is what a block composed before that field existed shows. Named, they are drawn in the order they were typed and not the catalog's, every serie of a site commonly sitting at position 0. A slug naming no serie is left out rather than widening the row, so a typo shows. That block also says how each serie is drawn: the card the listing has always printed — the cover with its title under it — a tile the cover fills whole with the text laid over its foot, or a square vignette. The two new ones also print how many books the serie has out and the language it reads in.

Each of the seven listing kinds — the six above and `book_stores` — carries its own head: an anchor, an eyebrow, a title, a paragraph written in the rich text editor and a colored flat, the very fields a `text_section` offers (`AbstractBookListingBlockType`, drawn by the `Listing:Section` component). Written on the listing itself rather than in a text block placed above it: the two were one section on the page and two rows in the back office, and the pair had to be moved, hidden and translated together. Left empty, the listing renders as a bare grid of covers, which is what a page composed before those fields existed shows.

Each may also close its head on a link — a label and an address, both needed — laid against the far edge of the title's own line. That is where a listing showing a cut of a catalog points at the whole of it, `displayMore` closing the grid with a card instead: a listing offers either, never both.
...
The eighth, `book_stores`, says where the catalog is sold — the platforms the site declares, each with its own mark and its own name, in the very tile a book's page prints its own links in. It holds its own items rather than querying anything, so it names one address per platform: the publisher's page on that platform, where a book's links point at that book. The platforms offered are the site's own vocabulary (`BookCustomizationProviderInterface::getLinkKinds()`), so a rail and a book page never name the same platform differently, and a platform the vocabulary gives no mark to is left out rather than drawn as a broken image. It draws either the tile a book's page prints, the name under the mark, or a wall of marks standing alone on wide plates — which is what a rail closing a page wants, the brands being read at a glance.
